...
#pragma region SharedComponentStorage
	...
	template <typename T>
	SCCollection* SharedComponentStorage::GetSCCollection()
	{
		...
		if (search != _SCCollectionsMap.end())
		{
			...
			return static_cast<SCCollection*>(_SCCollectionsList[index]);
		}
		return nullptr;
	}
	...
	template <typename T>
	std::vector<OwnersCollection*>* SharedComponentStorage::GetOwnersCollectionsList()
	{
		SCCollection* scc = GetSCCollection<T>();
		if (scc == nullptr) return nullptr;
	...
	template <typename T>
	std::vector<T*>* SharedComponentStorage::GetSCList()
	{
		SCCollection* scc = GetSCCollection<T>();
		if (scc == nullptr) return nullptr;
		return static_cast<std::vector<T*>*>(static_cast<void*>(&scc->_SCList));
	}

	template <typename T>
	OwnersCollection* SharedComponentStorage::GetOwnersCollection(T* value)
	{
		SCCollection* scc = GetSCCollection<T>();
		if (scc == nullptr) return nullptr;
		...
		{
			Index index = search->second;
			return static_cast<OwnersCollection*>(scc->_OwnersCollectionsList[index]);
		}
		return nullptr;
	}

	template <typename T>
	OwnersMap* SharedComponentStorage::GetOwnersMap(T* value)
	{
	...
	template <typename T>
	OwnersList* SharedComponentStorage::GetOwnersList(T* value)
	{
		OwnersCollection* oc = GetOwnersCollection<T>(value);
		if (oc == nullptr) return nullptr;
	...
	template <typename T>
	void SharedComponentStorage::SetSharedComponent(Entity entity, T* value)
	{
		SCCollection* scc = GetSCCollection<T>();
		...
		for (size_t i = 0; i < ocSize; i++)
		{
			...
			if (entitySearch != oc->_OwnersMap.end())
			{
				...
				if (oc->_OwnersList.size() == 1)
				{
					...
					scc->_OwnersCollectionsMap.at(
						dynamic_cast<SharedComponentBase*>(scc->_SCList.back())->GetHashCode()) = index;
	...
	template <typename T>
	T* SharedComponentStorage::GetSharedComponent(Entity entity)
	{
		SCCollection* scc = GetSCCollection<T>();
		if (scc == nullptr) return nullptr;
		size_t ocSize = scc->_OwnersCollectionsList.size();
