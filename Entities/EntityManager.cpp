...
std::vector<WorldEntityStorage*> EntityManager::_WorldEntityStorage;
WorldEntityStorage* EntityManager::_CurrentActivitedWorldEntityStorage;
std::vector<EntityInfo>* EntityManager::_EntityInfos;
std::vector<Entity>* EntityManager::_Entities;
std::vector<Entity>* EntityManager::_ParentRoots;
std::vector<EntityComponentStorage>* EntityManager::_EntityComponentStorage;
//std::vector<std::queue<Entity>>* UniEngine::EntityManager::_EntityPool;
SharedComponentStorage* EntityManager::_EntitySharedComponentStorage;
std::vector<EntityQuery>* EntityManager::_EntityQueries;
std::vector<EntityQueryInfo>* EntityManager::_EntityQueryInfos;
std::queue<EntityQuery>* EntityManager::_EntityQueryPools;

ThreadPool* EntityManager::_ThreadPool;
#pragma region EntityManager

void EntityManager::ForEachComponentUnsafe(Entity entity,
                                           const std::function<void(ComponentType type, void* data)>& func)
{
	...
	if (_Entities->at(entity.Index) == entity)
	{
		...
		{
			func(i.second,
			     static_cast<void*>(static_cast<char*>(chunk.Data) + i.second.Offset * chunkInfo->ChunkCapacity +
				     chunkPointer * i.second
				                     .Size));
		}
	}
}

void EntityManager::ForEachEntityStorageUnsafe(const std::function<void(int i, EntityComponentStorage storage)>& func)
{
...
void EntityManager::DeleteEntityInternal(Entity entity)
{
	EntityInfo info = _EntityInfos->at(entity.Index);
...
void EntityManager::RefreshEntityQueryInfos(size_t index)
{
	...
	if (!_EntityQueryInfos->at(index).AnyComponentTypes.empty())
	{
		for (int i = 0; i < _EntityQueryInfos->at(index).QueriedStorages.size(); i++)
		{
			...
			{
				auto search = _EntityQueryInfos->at(index).QueriedStorages.at(i).ArchetypeInfo->ComponentTypes.find(
					type.TypeID);
				if (search != _EntityQueryInfos->at(index).QueriedStorages.at(i).ArchetypeInfo->ComponentTypes.end())
					contain = true;
				if (contain) break;
			}
			if (!contain)
			{
				_EntityQueryInfos->at(index).QueriedStorages.erase(
					_EntityQueryInfos->at(index).QueriedStorages.begin() + i);
				i--;
			}
		}
	}
...
void EntityManager::GetEntityStorage(EntityComponentStorage storage, std::vector<Entity>* container)
{
...
size_t EntityManager::SwapEntity(EntityComponentStorage storage, size_t index1, size_t index2)
{
	...
	{
		void* temp = static_cast<void*>(malloc(i.second.Size));
...
std::vector<Entity>* EntityManager::GetParentRootsUnsafe()
{
	return _ParentRoots;
}

void EntityManager::Init(ThreadPool* threadPool)
{
...
std::vector<Entity>* EntityManager::GetAllEntitiesUnsafe()
{
	return _Entities;
}

void EntityManager::SetThreadPool(ThreadPool* pool)
{
	_ThreadPool = pool;
}
...
Entity EntityManager::CreateEntity(EntityArchetype archetype)
{
	...
	else
	{
		...
		ComponentDataChunk chunk = _EntityComponentStorage->at(entityInfo.ArchetypeInfoIndex).ChunkArray->Chunks[
			chunkIndex];
		size_t offset = 0;
		bool found = false;
		for (const auto& i : chunkInfo->ComponentTypes)
...
void EntityManager::SetParent(Entity entity, Entity parent)
{
...
size_t EntityManager::GetChildrenAmount(Entity entity)
{
...
inline void EntityManager::ForEachChild(Entity entity, const std::function<void(Entity child)>& func)
{
...
void EntityManager::RemoveChild(Entity entity, Entity parent)
{
	if (entity.IsNull() || parent.IsNull()) return;
	size_t childIndex = entity.Index;
	size_t parentIndex = parent.Index;
...
void EntityManager::GetParentRoots(std::vector<Entity>* container)
{
...
EntityArchetype EntityManager::GetEntityArchetype(Entity entity)
{
...
void EntityManager::SetEnable(Entity entity, bool value)
{
...
bool EntityManager::IsEntityEnabled(Entity entity)
{
	return _EntityInfos->at(entity.Index).Enabled;
}

bool EntityManager::IsEntityDeleted(size_t index)
{
...
void EntityManager::DeleteEntityQuery(EntityQuery entityQuery)
{
...
std::vector<EntityComponentStorage> EntityManager::UnsafeQueryStorages(EntityQuery entityQuery)
{
...
ComponentDataChunkArray* EntityManager::UnsafeGetEntityComponentDataChunkArray(EntityArchetype entityArchetype)
{
...
inline void EntityManager::ForAllRootParent(const std::function<void(int i, Entity rootParent)>& func)
{
...
EntityComponentStorage::EntityComponentStorage(EntityArchetypeInfo* info, ComponentDataChunkArray* array)
{
	ArchetypeInfo = info;
	ChunkArray = array;
}
...
size_t EntityQuery::GetEntityAmount()
{
