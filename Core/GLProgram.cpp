...
inline void GLProgram::BindDefault()
{
	glUseProgram(0);
}
...
inline GLProgram::GLProgram(GLShader* vertexShader, GLShader* fragmentShader, GLShader* geometryShader)
{
	_ID = glCreateProgram();
	_VertexShader = nullptr;
	_FragmentShader = nullptr;
...
inline void GLProgram::Attach(ShaderType type, GLShader* shader)
{
	switch (type)
	{
...
inline void GLProgram::Detach(ShaderType type)
{
	switch (type)
	{
	case ShaderType::Vertex:
