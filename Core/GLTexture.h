...
namespace UniEngine
{
	...
	public:
		...
		{
			if (static_cast<GLint>(activate) >= _MaxAllowedTexture)
			{
	...
	public:
		            ...
		            GLsizei width, bool immutable = true) : GLTexture(GL_TEXTURE_1D, internalFormat)
		{
			...
			Bind(0);
			if (immutable)
				glTexStorage1D(_Type, levels, internalFormat, width);
		};
	...
	public:
		            ...
		            GLsizei height, bool immutable = true) : GLTexture(GL_TEXTURE_2D, internalFormat)
		{
			...
			Bind(0);
			if (immutable)
				glTexStorage2D(_Type, levels, internalFormat, width, height);
			else
				glTexImage2D(_Type, levels, internalFormat, width, height, 0, GL_RED, GL_FLOAT, nullptr);
		};
	...
	public:
		            ...
		            GLsizei depth, bool immutable = true) : GLTexture(GL_TEXTURE_3D, internalFormat)
		{
			...
			Bind(0);
			if (immutable)
				glTexStorage3D(_Type, levels, internalFormat, width, height, depth);
		};
	...
	public:
		...
		void SetData(CubeMapIndex index,
		             ...
		             const void* pixels)
		{
			if (static_cast<size_t>(index) > 5)
			{
				Debug::Error("Error!");
			}
			Bind(0);
			glTexSubImage3D(_Type, level, 0, 0, static_cast<size_t>(index), _Width, _Height, 1, format, type, pixels);
		}
		             ...
		             const void* pixels)
		{
			...
			Bind(0);
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + static_cast<int>(index), level, internalFormat, _Width,
			             _Height, 0, format, type, pixels);
		}
		...
		void SubData(CubeMapIndex index,
		             ...
		             const void* pixels)
		{
			if ((xoffset + width) > _Width || (yoffset + height) > _Height || static_cast<size_t>(index) > 5)
			{
				Debug::Error("Error!");
			}
			Bind(0);
			glTexSubImage3D(_Type, level, xoffset, yoffset, static_cast<size_t>(index), width, height, 1, format, type,
			                pixels);
		}
	};
	...
	public:
		             ...
		             const void* pixels)
		{
			...
			Bind(0);
			glTexSubImage3D(_Type, level, xoffset, yoffset, layer * 6 + static_cast<size_t>(index), width, height, 1,
			                format, type, pixels);
		}
	};
