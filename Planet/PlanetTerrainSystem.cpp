...
void Planet::PlanetTerrainSystem::GenerateTerrain(PlanetTerrain* planetTerrain, TerrainChunk* targetChunk)
{
	...
	{
		int actualDetailLevel = static_cast<int>(glm::pow(2, targetChunk->DetailLevel));
		int x = index % resolution;
		int y = index / resolution;
		glm::dvec2 percent = glm::dvec2(x, y) / static_cast<double>(resolution - 1) / static_cast<double>(
			actualDetailLevel);
		glm::dvec3 pointOnUnitCube = targetChunk->LocalUp + (percent.x + static_cast<double>(targetChunk->
			ChunkCoordinate.x) / actualDetailLevel - 0.5) * 2 * targetChunk->AxisA + (percent.y + static_cast<double>(
			targetChunk->ChunkCoordinate.y) / actualDetailLevel - 0.5) * 2 * targetChunk->AxisB;
...
void Planet::PlanetTerrainSystem::Update()
{
	...
	for (auto i = 0; i < _PlanetTerrainList.size(); i++)
	{
		...
		glm::mat4 matrix = scale(translate(mat4_cast(planetInfo.Rotation), glm::vec3(planetInfo.Position)),
		                         glm::vec3(1.0f));
	...
	for (auto i = 0; i < _PlanetTerrainList.size(); i++)
	{
		...
		for (auto j = 0; j < planetTerrain->_ChunkList.size(); j++)
		{
			...
			if (distance(
					glm::vec3(chunk->ChunkCenterPosition(planetInfo.Position, planetInfo.Radius, planetInfo.Rotation)),
					cameraPos.Value)
				< (planetInfo.LodDistance * planetInfo.Radius / glm::pow(2, chunk->DetailLevel + 1)))
			{
				if (chunk->DetailLevel < planetInfo.MaxLodLevel)
				{
					...
					{
						//TODO: Dont forget to reset index.
						chunk->C0 = new TerrainChunk(chunk, 0, chunk->DetailLevel + 1,
						                             glm::ivec2(chunk->ChunkCoordinate.x * 2,
						                                        chunk->ChunkCoordinate.y * 2 + 1),
						                             ChunkDirection::UpperLeft, chunk->LocalUp);
						_GenerationQueue.push(chunk->C0);
					}
					if (chunk->C1 != nullptr)
					{
					...
					{
						chunk->C1 = new TerrainChunk(chunk, 0, chunk->DetailLevel + 1,
						                             glm::ivec2(chunk->ChunkCoordinate.x * 2 + 1,
						                                        chunk->ChunkCoordinate.y * 2 + 1),
						                             ChunkDirection::UpperRight, chunk->LocalUp);
						_GenerationQueue.push(chunk->C1);
					}
					if (chunk->C2 != nullptr)
					{
					...
					{
						chunk->C2 = new TerrainChunk(chunk, 0, chunk->DetailLevel + 1,
						                             glm::ivec2(chunk->ChunkCoordinate.x * 2,
						                                        chunk->ChunkCoordinate.y * 2),
						                             ChunkDirection::LowerLeft, chunk->LocalUp);
						_GenerationQueue.push(chunk->C2);
					}
					if (chunk->C3 != nullptr)
					{
					...
					{
						chunk->C3 = new TerrainChunk(chunk, 0, chunk->DetailLevel + 1,
						                             glm::ivec2(chunk->ChunkCoordinate.x * 2 + 1,
						                                        chunk->ChunkCoordinate.y * 2),
						                             ChunkDirection::LowerRight, chunk->LocalUp);
						_GenerationQueue.push(chunk->C3);
					}
