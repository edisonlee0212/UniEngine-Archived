...
void CameraControlSystem::Update()
{
	...
	if (InputManager::GetMouse(GLFW_MOUSE_BUTTON_RIGHT)) {
		if (xoffset != 0 || yoffset != 0) {
			rc.Value = Application::GetMainCameraComponent()->get()->GetCamera()->ProcessMouseMovement(xoffset, yoffset, _Sensitivity);
		...
		_LastScrollY = mousePosition.y;
		if (yscrolloffset != 0) Application::GetMainCameraComponent()->get()->GetCamera()->ProcessMouseScroll(yscrolloffset);
	}
}
