...
{
	EntityManager::ForEach<LocalToParent, LocalTranslation>(
		_LP, [](int i, Entity entity, LocalToParent* ltp, LocalTranslation* lp)
		{
			ltp->Value = translate(glm::mat4(1.0f), lp->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalTranslation, LocalRotation>(
		_LPR, [](int i, Entity entity, LocalToParent* ltp, LocalTranslation* lp, LocalRotation* lr)
		{
			ltp->Value = translate(glm::mat4(1.0f), lp->Value) * mat4_cast(lr->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalTranslation, LocalScale>(
		_LPS, [](int i, Entity entity, LocalToParent* ltp, LocalTranslation* lp, LocalScale* ls)
		{
			ltp->Value = translate(glm::mat4(1.0f), lp->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalTranslation, LocalRotation, LocalScale>(
		_LPRS, [](int i, Entity entity, LocalToParent* ltp, LocalTranslation* lp, LocalRotation* lr, LocalScale* ls)
		{
			ltp->Value = translate(glm::mat4(1.0f), lp->Value) * mat4_cast(lr->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalRotation>(
		_LR, [](int i, Entity entity, LocalToParent* ltp, LocalRotation* lr)
		{
			ltp->Value = mat4_cast(lr->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalRotation, LocalScale>(
		_LRS, [](int i, Entity entity, LocalToParent* ltp, LocalRotation* lr, LocalScale* ls)
		{
			ltp->Value = mat4_cast(lr->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToParent, LocalScale>(_LS, [](int i, Entity entity, LocalToParent* ltp, LocalScale* ls)
	{
		ltp->Value = scale(ls->Value);
	});

	EntityManager::ForEach<LocalToWorld, Translation>(_P, [](int i, Entity entity, LocalToWorld* ltw, Translation* lp)
	{
		ltw->Value = translate(glm::mat4(1.0f), lp->Value);
	});
	EntityManager::ForEach<LocalToWorld, Translation, Rotation>(
		_PR, [](int i, Entity entity, LocalToWorld* ltw, Translation* lp, Rotation* lr)
		{
			ltw->Value = translate(glm::mat4(1.0f), lp->Value) * mat4_cast(lr->Value);
		});
	EntityManager::ForEach<LocalToWorld, Translation, Scale>(
		_PS, [](int i, Entity entity, LocalToWorld* ltw, Translation* lp, Scale* ls)
		{
			ltw->Value = translate(glm::mat4(1.0f), lp->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToWorld, Translation, Rotation, Scale>(
		_PRS, [](int i, Entity entity, LocalToWorld* ltw, Translation* lp, Rotation* lr, Scale* ls)
		{
			ltw->Value = translate(glm::mat4(1.0f), lp->Value) * mat4_cast(lr->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToWorld, Rotation>(_R, [](int i, Entity entity, LocalToWorld* ltw, Rotation* lr)
	{
		ltw->Value = mat4_cast(lr->Value);
	});
	EntityManager::ForEach<LocalToWorld, Rotation, Scale>(
		_RS, [](int i, Entity entity, LocalToWorld* ltw, Rotation* lr, Scale* ls)
		{
			ltw->Value = mat4_cast(lr->Value) * scale(ls->Value);
		});
	EntityManager::ForEach<LocalToWorld, Scale>(_S, [](int i, Entity entity, LocalToWorld* ltw, Scale* ls)
	{
		ltw->Value = scale(ls->Value);
	});
...
void TransformSystem::CalculateLTWRecursive(LocalToWorld pltw, Entity entity)
{
	/*
...
void TransformSystem::CollectHierarchy(std::vector<std::pair<Entity, ChildInfo>>* container, Entity entity)
{
	auto children = EntityManager::GetChildren(entity);
	auto initialSize = container->size();
	for (const auto& i : children)
	{
		if (EntityManager::HasComponentData<LocalToWorld>(i) && EntityManager::HasComponentData<LocalToParent>(i) &&
			EntityManager::HasComponentData<LocalToWorld>(entity))
		{
			ChildInfo info;
			info.Child = i;
			auto ltp = EntityManager::GetComponentData<LocalToParent>(i);
	...
	for (int i = initialSize; i < container->size(); i++)
	{
		...
		{
			if (EntityManager::HasComponentData<LocalToWorld>(child) && EntityManager::HasComponentData<LocalToParent
			>(child) && EntityManager::HasComponentData<LocalToWorld>(target))
			{
				ChildInfo info;
				info.Child = child;
