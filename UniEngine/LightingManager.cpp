...
#pragma region LightInfoBlocks
	...
	_DirectionalLightBlock->SetData(static_cast<GLsizei>(size), nullptr, static_cast<GLsizei>(GL_DYNAMIC_DRAW));
	_DirectionalLightBlock->SetBase(1);
	size = 16 + Default::ShaderIncludes::MaxPointLightAmount * sizeof(PointLight);
	_PointLightBlock->SetData(static_cast<GLsizei>(size), nullptr, static_cast<GLsizei>(GL_DYNAMIC_DRAW));
	_PointLightBlock->SetBase(2);
	size = 16 + Default::ShaderIncludes::MaxSpotLightAmount * sizeof(SpotLight);
	_SpotLightBlock->SetData(static_cast<GLsizei>(size), nullptr, static_cast<GLsizei>(GL_DYNAMIC_DRAW));
	_SpotLightBlock->SetBase(3);
#pragma endregion
#pragma region DirectionalLight
	_DirectionalLightShadowMap = new DirectionalLightShadowMap(1, _DirectionalShadowMapResolution,
	                                                           _DirectionalShadowMapResolution,
	                                                           _ShadowSettings.SoftShadowMode == static_cast<int>(
		                                                           ShadowMode::VSM));
	...
	_DirectionalLightShadowMapFilter = new RenderTarget(_DirectionalShadowMapResolution,
	                                                    _DirectionalShadowMapResolution);
	float borderColor[] = {1.0f, 1.0f, 1.0f, 1.0f};
	_DLVSMVFilter = new GLTexture2DArray(1, GL_RG32F, static_cast<GLsizei>(_DirectionalShadowMapResolution),
	                                     static_cast<GLsizei>(_DirectionalShadowMapResolution),
	                                     static_cast<GLsizei>(Default::ShaderIncludes::ShadowCascadeAmount));
...
#pragma region DirectionalLight data collection
				...
				glm::vec3 lightDir = normalize(rotation * glm::vec3(0, 0, 1));
...
#pragma endregion
				for (int split = 0; split < Default::ShaderIncludes::ShadowCascadeAmount; split++)
				{
					...
					if (split != Default::ShaderIncludes::ShadowCascadeAmount - 1) splitEnd = _MaxShadowDistance *
						_ShadowCascadeSplit[split];
...
#pragma region AABB
					...
					glm::vec3 cameraFrustumCenter = (cameraRot * glm::vec3(0, 0, -1)) * ((splitEnd - splitStart) / 2.0f
						+ splitStart) + cameraPos;
					if (_StableFit)
					{
					...
					else
					{
						...
						max = glm::max(max, distance(cornerPoints[0],
						                             ClosestPointOnLine(cornerPoints[0], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[1],
						                             ClosestPointOnLine(cornerPoints[1], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[2],
						                             ClosestPointOnLine(cornerPoints[2], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[3],
						                             ClosestPointOnLine(cornerPoints[3], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[4],
						                             ClosestPointOnLine(cornerPoints[4], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[5],
						                             ClosestPointOnLine(cornerPoints[5], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[6],
						                             ClosestPointOnLine(cornerPoints[6], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
						max = glm::max(max, distance(cornerPoints[7],
						                             ClosestPointOnLine(cornerPoints[7], cameraFrustumCenter,
						                                                cameraFrustumCenter - lightDir)));
					}


					glm::vec3 p0 = ClosestPointOnLine(glm::vec3(maxBound.x, maxBound.y, maxBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);
					glm::vec3 p7 = ClosestPointOnLine(glm::vec3(minBound.x, minBound.y, minBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);

					float d0 = distance(p0, p7);

					glm::vec3 p1 = ClosestPointOnLine(glm::vec3(maxBound.x, maxBound.y, minBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);
					glm::vec3 p6 = ClosestPointOnLine(glm::vec3(minBound.x, minBound.y, maxBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);

					float d1 = distance(p1, p6);

					glm::vec3 p2 = ClosestPointOnLine(glm::vec3(maxBound.x, minBound.y, maxBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);
					glm::vec3 p5 = ClosestPointOnLine(glm::vec3(minBound.x, maxBound.y, minBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);

					float d2 = distance(p2, p5);

					glm::vec3 p3 = ClosestPointOnLine(glm::vec3(maxBound.x, minBound.y, minBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);
					glm::vec3 p4 = ClosestPointOnLine(glm::vec3(minBound.x, maxBound.y, maxBound.z),
					                                  cameraFrustumCenter, cameraFrustumCenter + lightDir);
					...
					lightView = lookAt(lightPos, lightPos + lightDir, glm::vec3(0.0, 1.0, 0.0));
					if (any(isnan(lightView[3])))
					{
						lightView = lookAt(lightPos, lightPos + lightDir, glm::vec3(0.0, 0.0, 1.0));
					}
...
#pragma region Fix Shimmering due to the movement of the camera
					...
					shadowOrigin = shadowOrigin * static_cast<float>(_DirectionalShadowMapResolution) / 2.0f;
					glm::vec4 roundedOrigin = round(shadowOrigin);
					glm::vec4 roundOffset = roundedOrigin - shadowOrigin;
					roundOffset = roundOffset * 2.0f / static_cast<float>(_DirectionalShadowMapResolution);
					roundOffset.z = 0.0f;
					roundOffset.w = 0.0f;
...
#pragma endregion
					...
					if (split == Default::ShaderIncludes::ShadowCascadeAmount - 1) _DirectionalLights[i].
						ReservedParameters = glm::vec4(dlc->lightSize, 0, dlc->depthBias, dlc->normalOffset);
				}
			}
#pragma endregion
...
#pragma region Directional Light Shadowmap pass
			...
			_DirectionalLightShadowMap->SetVSM(_ShadowSettings.SoftShadowMode == static_cast<int>(ShadowMode::VSM));
			_DirectionalLightShadowMap->Bind();
			glEnable(GL_DEPTH_TEST);
			if (_ShadowSettings.SoftShadowMode == static_cast<int>(ShadowMode::VSM))
			{
				glClearTexSubImage(_DirectionalLightShadowMap->DepthMapArray()->ID(), 0, 0, 0, 0,
				                   static_cast<GLsizei>(_DirectionalShadowMapResolution),
				                   static_cast<GLsizei>(_DirectionalShadowMapResolution),
				                   static_cast<GLsizei>(size) * 4, GL_RG, GL_FLOAT, nullptr);
			}
			else
			{
				glClearTexSubImage(_DirectionalLightShadowMap->DepthMapArray()->ID(), 0, 0, 0, 0,
				                   static_cast<GLsizei>(_DirectionalShadowMapResolution),
				                   static_cast<GLsizei>(_DirectionalShadowMapResolution),
				                   static_cast<GLsizei>(size) * 4, GL_RED, GL_FLOAT, nullptr);
			}
			for (int i = 0; i < size; i++)
			{
						...
						if (mmc->_CastShadow)
						{
							...
							for (auto& j : *entities)
							{
								...
								*/
								...
								glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(mesh->Size()), GL_UNSIGNED_INT,
								               nullptr);
							}
						}
					}
				...
				if (instancedMeshMaterials != nullptr)
				{
					for (auto immc : *instancedMeshMaterials)
					{
						...
						if (immc->_CastShadow)
						{
							auto entities = EntityManager::GetSharedComponentEntities<InstancedMeshMaterialComponent>(
								immc);
							size_t count = immc->_Matrices->size();
							GLVBO* matricesBuffer = new GLVBO();
							matricesBuffer->SetData(static_cast<GLsizei>(count) * sizeof(glm::mat4),
							                        &immc->_Matrices->at(0), GL_STATIC_DRAW);
							for (auto& entity : *entities)
							{
								if (!entity.Enabled()) continue;
								auto mesh = immc->_Mesh;
								_DirectionalLightInstancedProgram->SetFloat4x4(
									"model", EntityManager::GetComponentData<LocalToWorld>(entity).Value);
								mesh->Enable();
								mesh->VAO()->EnableAttributeArray(12);
								mesh->VAO()->SetAttributePointer(12, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
								                                 static_cast<void*>(nullptr));
								mesh->VAO()->EnableAttributeArray(13);
								mesh->VAO()->SetAttributePointer(13, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
								                                 (void*)(sizeof(glm::vec4)));
								mesh->VAO()->EnableAttributeArray(14);
								mesh->VAO()->SetAttributePointer(14, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
								                                 (void*)(2 * sizeof(glm::vec4)));
								mesh->VAO()->EnableAttributeArray(15);
								mesh->VAO()->SetAttributePointer(15, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
								                                 (void*)(3 * sizeof(glm::vec4)));
								mesh->VAO()->SetAttributeDivisor(12, 1);
								mesh->VAO()->SetAttributeDivisor(13, 1);
								mesh->VAO()->SetAttributeDivisor(14, 1);
								mesh->VAO()->SetAttributeDivisor(15, 1);
								glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(mesh->Size()),
...
#pragma region VSM Filter Pass
			...
			_DirectionalLightShadowMapFilter->AttachTexture(_DLVSMVFilter, GL_COLOR_ATTACHMENT0);
			_DirectionalLightShadowMapFilter->AttachTexture(_DirectionalLightShadowMap->DepthMapArray(),
...
#pragma region Prepare PointLight data
			...
			for (int i = 0; i < size; i++)
			{
				...
				glm::mat4 shadowProj = glm::perspective(glm::radians(90.0f), _PointLightShadowMap->GetResolutionRatio(),
				                                        1.0f, _PointLights[i].constantLinearQuadFarPlane.w);
				_PointLights[i].lightSpaceMatrix[0] = shadowProj * lookAt(
					position, position + glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f));
				_PointLights[i].lightSpaceMatrix[1] = shadowProj * lookAt(
					position, position + glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f));
				_PointLights[i].lightSpaceMatrix[2] = shadowProj * lookAt(
					position, position + glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
				_PointLights[i].lightSpaceMatrix[3] = shadowProj * lookAt(
					position, position + glm::vec3(0.0f, -1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f));
				_PointLights[i].lightSpaceMatrix[4] = shadowProj * lookAt(
					position, position + glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, -1.0f, 0.0f));
				_PointLights[i].lightSpaceMatrix[5] = shadowProj * lookAt(
					position, position + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f));
				...
				for (auto mmc : *meshMaterials)
				{
					...
					if (mmc->_CastShadow)
					{
						...
						for (auto& entity : *entities)
						{
							if (!entity.Enabled()) continue;
							auto mesh = mmc->_Mesh;
							_PointLightProgram->SetFloat4x4(
								"model", EntityManager::GetComponentData<LocalToWorld>(entity).Value);
							mesh->Enable();
							mesh->VAO()->DisableAttributeArray(12);
							mesh->VAO()->DisableAttributeArray(13);
					...
					for (auto immc : *instancedMeshMaterials)
					{
						...
						if (immc->_CastShadow)
						{
							...
							for (auto& entity : *entities)
							{
								if (!entity.Enabled()) continue;
								auto mesh = immc->_Mesh;
								_PointLightInstancedProgram->SetFloat4x4(
									"model", EntityManager::GetComponentData<LocalToWorld>(entity).Value);
								mesh->Enable();
								mesh->VAO()->EnableAttributeArray(12);
								mesh->VAO()->SetAttributePointer(12, 4, GL_FLOAT, GL_FALSE, sizeof(glm::mat4),
...
void LightingManager::SetSplitRatio(float r1, float r2, float r3, float r4)
{
...
void LightingManager::SetDirectionalLightResolution(size_t value)
{
	_DirectionalShadowMapResolution = value;
}

void LightingManager::SetShadowMode(ShadowMode value)
{
	_ShadowSettings.SoftShadowMode = static_cast<int>(value);
}


void LightingManager::SetEnableSplitDisplay(bool value)
{
	_ShadowSettings.DisplaySplit = value ? 1.0f : 0.0f;
}

void LightingManager::SetStableFit(bool value)
{
	_StableFit = true;
}

void LightingManager::SetSeamFixRatio(float value)
{
	_ShadowSettings.SeamFixRatio = value;
}

void LightingManager::SetMaxShadowDistance(float value)
{
	_MaxShadowDistance = value;
}

void LightingManager::SetVSMMaxVariance(float value)
{
	_ShadowSettings.VSMMaxVariance = value;
}

void LightingManager::SetLightBleedControlFactor(float value)
{
	_ShadowSettings.LightBleedFactor = value;
}

void LightingManager::SetPCSSScaleFactor(float value)
{
	_ShadowSettings.PCSSScaleFactor = value;
}

void LightingManager::SetEVSMExponent(float value)
{
	_ShadowSettings.EVSMExponent = value;
}

void LightingManager::SetAmbientLight(float value)
{
	_ShadowSettings.AmbientLight = value;
}

glm::vec3 LightingManager::ClosestPointOnLine(glm::vec3 point, glm::vec3 a, glm::vec3 b)
{
