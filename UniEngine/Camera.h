...
namespace UniEngine {
	...
	public:
		...
		void CalculateFrustumPoints(glm::vec3 cameraPos, glm::vec3* points, float nearPlane, float farPlane);
		...
		glm::mat4 _View;
		Camera(RenderTarget* renderTarget, float yaw = YAW, float pitch = PITCH, float nearPlane = 0.1f, float farPlane = 50.0f);
