...
void UniEngine::EntityManager::UnsafeForEachComponent(const Entity& entity, const std::function<void(const ComponentType& type, void* data)>& func)
{
	...
	if (GetInstance().m_entities->at(entity.m_index) == entity) {
		...
		for (const auto& i : chunkInfo->m_componentTypes) {
			func(i, static_cast<void*>(static_cast<char*>(chunk.m_data) + i.m_offset * chunkInfo->m_chunkCapacity + chunkPointer * i.m_size));
		}
	}
}
...
void UniEngine::EntityManager::DeleteEntityInternal(const Entity& entity)
{
	...
	if (actualEntity == entity) {
		...
		if (info.m_chunkArrayIndex != storage.m_archetypeInfo->m_entityAliveCount - 1) {
			const auto swappedIndex = SwapEntity(storage, info.m_chunkArrayIndex, storage.m_archetypeInfo->m_entityAliveCount - 1);
...
void UniEngine::EntityManager::RefreshEntityQueryInfos(const size_t& index)
{
	...
	if (!GetInstance().m_entityQueryInfos->at(index).m_allComponentTypes.empty()) {
		for (auto i : *GetInstance().m_entityComponentStorage) {
			if (i.m_archetypeInfo == nullptr) continue;
			bool check = true;
			for (const auto& type : GetInstance().m_entityQueryInfos->at(index).m_allComponentTypes) {
				if (!i.m_archetypeInfo->HasType(type.m_typeId)) check = false;
			}
			if (check) GetInstance().m_entityQueryInfos->at(index).m_queriedStorage.push_back(i);
		}
	}
	else {
		for (auto i : *GetInstance().m_entityComponentStorage) {
	...
	if (!GetInstance().m_entityQueryInfos->at(index).m_anyComponentTypes.empty()) {
		for (int i = 0; i < GetInstance().m_entityQueryInfos->at(index).m_queriedStorage.size(); i++) {
			...
			for (const auto& type : GetInstance().m_entityQueryInfos->at(index).m_anyComponentTypes) {
				if (GetInstance().m_entityQueryInfos->at(index).m_queriedStorage.at(i).m_archetypeInfo->HasType(type.m_typeId)) contain = true;
				if (contain) break;
			}
			if (!contain) {
				GetInstance().m_entityQueryInfos->at(index).m_queriedStorage.erase(GetInstance().m_entityQueryInfos->at(index).m_queriedStorage.begin() + i);
				i--;
			}
		}
	}
...
size_t UniEngine::EntityManager::SwapEntity(const EntityComponentStorage& storage, size_t index1, size_t index2)
{
	...
	for (const auto& i : storage.m_archetypeInfo->m_componentTypes) {
		...
		void* d1 = static_cast<void*>(static_cast<char*>(storage.m_chunkArray->Chunks[chunkIndex1].m_data)
			+ i.m_offset * capacity
			+ i.m_size * chunkPointer1);

		void* d2 = static_cast<void*>(static_cast<char*>(storage.m_chunkArray->Chunks[chunkIndex2].m_data)
			+ i.m_offset * capacity
...
Entity UniEngine::EntityManager::CreateEntity(const EntityArchetype& archetype, const std::string& name)
{
	...
	if (info->m_entityCount == info->m_entityAliveCount) {
		...
		const auto chunk = GetInstance().m_entityComponentStorage->at(entityInfo.m_archetypeInfoIndex).m_chunkArray->Chunks[chunkIndex];
...
std::vector<Entity> EntityManager::CreateEntities(const EntityArchetype& archetype, const size_t& amount, const std::string& name)
{
	...
	while(remainAmount > 0 && info->m_entityAliveCount != info->m_entityCount)
	{
		...
		const ComponentDataChunk chunk = GetInstance().m_entityComponentStorage->at(entityInfo.m_archetypeInfoIndex).m_chunkArray->Chunks[chunkIndex];
		for (const auto& i : info->m_componentTypes) {
...
void UniEngine::EntityManager::DeleteEntity(const Entity& entity)
{
	...
	if (GetInstance().m_entityInfos->at(entityIndex).m_parent.m_index != 0) RemoveChild(entity, GetInstance().m_entityInfos->at(entityIndex).m_parent);
	...
	PrivateComponentBase* ptr)
{
	...
	if (!found)
	{
		...
		GetInstance().m_entityInfos->at(entity.m_index).m_privateComponentElements.emplace_back(name, id, std::unique_ptr<PrivateComponentBase>(ptr), entity);
	}
}
...
{
	if (index > 0 && index < GetInstance().m_entities->size()) return GetInstance().m_entities->at(index);
	return Entity();
}
...
{
	return GetInstance().m_currentAttachedWorldEntityStorage->m_entityComponentStorage[entityArchetype.m_index].m_archetypeInfo->m_name;
}
