...
#pragma region Get transforms from physX
	if (rigidBodyEntities)
	{
		...
		for (size_t i = 0; i < threadSize; i++) {
			futures.push_back(JobManager::PrimaryWorkers().Push([&list, i, capacity](int id)
				{
					for (size_t j = 0; j < capacity; j++)
					{
						...
						if (rigidBody->IsEnabled()) {
							...
							const glm::mat4 pltw = parentEntity.IsNull() ? GlobalTransform().m_value : parentEntity.GetComponentData<GlobalTransform>().m_value;
