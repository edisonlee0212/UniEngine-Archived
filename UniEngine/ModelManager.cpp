...
Model* ModelManager::LoadModel(std::string const& path, GLProgram* shader, bool gamma)
{
...
Entity ModelManager::ToEntity(EntityArchetype archetype, Model* model)
{
...
void ModelManager::ProcessNode(std::string directory, GLProgram* program, ModelNode* modelNode,
                               std::vector<Texture2D*>* Texture2DsLoaded, aiNode* node, const aiScene* scene)
{
...
void ModelManager::ReadMesh(unsigned meshIndex, ModelNode* modelNode, std::string directory, GLProgram* program,
                            std::vector<Texture2D*>* Texture2DsLoaded, aiMesh* aimesh, const aiScene* scene)
{
	unsigned mask = 1;
	...
	std::vector<Texture2D*> diffuseMaps = LoadMaterialTextures(directory, Texture2DsLoaded, pointMaterial,
	                                                           aiTextureType_DIFFUSE, TextureType::DIFFUSE);
	Texture2Ds->insert(Texture2Ds->end(), diffuseMaps.begin(), diffuseMaps.end());
	// 2. specular maps
	std::vector<Texture2D*> specularMaps = LoadMaterialTextures(directory, Texture2DsLoaded, pointMaterial,
	                                                            aiTextureType_SPECULAR, TextureType::SPECULAR);
	Texture2Ds->insert(Texture2Ds->end(), specularMaps.begin(), specularMaps.end());
	// 3. normal maps
	std::vector<Texture2D*> normalMaps = LoadMaterialTextures(directory, Texture2DsLoaded, pointMaterial,
...
std::vector<Texture2D*> ModelManager::LoadMaterialTextures(std::string directory,
                                                           std::vector<Texture2D*>* Texture2DsLoaded, aiMaterial* mat,
	...
	for (unsigned i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		for (unsigned j = 0; j < Texture2DsLoaded->size(); j++)
		{
			if (std::strcmp(Texture2DsLoaded->at(j)->Path().c_str(), str.C_Str()) == 0)
			{
				...
				skip = true;
				// a Texture2D with the same filepath has already been loaded, continue to next one. (optimization)
				break;
			}
		}
		if (!skip)
		{
			...
			// store it as Texture2D loaded for entire model, to ensure we won't unnecesery load duplicate Texture2Ds.
		}
	}
	return Texture2Ds;
}

void ModelManager::AttachChildren(EntityArchetype archetype, ModelNode* modelNode, Entity parentEntity)
{
	Entity entity = EntityManager::CreateEntity(archetype);
