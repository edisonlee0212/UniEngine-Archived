...
namespace UniEngine
{
	class UNIENGINE_API ModelManager : public ManagerBase
	{
		...
		static void ProcessNode(std::string, GLProgram* shader, ModelNode* modelNode, std::vector<Texture2D*>*, aiNode*,
		                        const aiScene*);
		static void ReadMesh(unsigned meshIndex, ModelNode* modelNode, std::string, GLProgram* shader,
		                     std::vector<Texture2D*>* texturesLoaded, aiMesh* mesh, const aiScene* scene);
		static std::vector<Texture2D*> LoadMaterialTextures(std::string, std::vector<Texture2D*>* texturesLoaded,
